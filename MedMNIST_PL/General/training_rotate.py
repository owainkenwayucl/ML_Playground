...
def generate_dataloaders(dataset, batch_size):
    ...
    data_transform90 = torchvision.transforms.Compose([
        torchvision.transforms.ToTensor(),
        torchvision.transforms.functional.rotate(angle=90),
        torchvision.transforms.Normalize(mean=[0.5], std=[0.5])
    ])

    data_transform180 = torchvision.transforms.Compose([
        torchvision.transforms.ToTensor(),
        torchvision.transforms.functional.rotate(angle=180),
        torchvision.transforms.Normalize(mean=[0.5], std=[0.5])
    ])
    ...
    data_transform270 = torchvision.transforms.Compose([
        torchvision.transforms.ToTensor(),
        torchvision.transforms.functional.rotate(angle=270),
        torchvision.transforms.Normalize(mean=[0.5], std=[0.5])
    ])

    train0 = data_class(split="train", transform=data_transform, download=True, size=224, mmap_mode='r')
    train90 = data_class(split="train", transform=data_transform90, download=True, size=224, mmap_mode='r')   
    train180 = data_class(split="train", transform=data_transform180, download=True, size=224, mmap_mode='r')
    train270 = data_class(split="train", transform=data_transform270, download=True, size=224, mmap_mode='r')   

    train = torch.utils.data.ConcatDataset([train0, train90, train180, train270])
    ...
    train_dataloader = torch.utils.data.DataLoader(dataset=train, batch_size = batch_size, shuffle=True, num_workers=20, persistent_workers=False)
    val_dataloader = torch.utils.data.DataLoader(dataset=val, batch_size = batch_size, shuffle=False, num_workers=20, persistent_workers=False)
    test_dataloader = torch.utils.data.DataLoader(dataset=test, batch_size = batch_size, shuffle=False, num_workers=20, persistent_workers=False)
